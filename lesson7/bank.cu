...
int main(int argc, char **argv)
{
    ...
    const int N = atoi(argv[1]); //拿到矩阵大小
 
    const int N2 = N * N; //矩阵的实际元素个数
    const int M = sizeof(real) * N2; //实际要开的内存
    real *h_A = (real *) malloc(M); //待转置矩阵A
    real *h_B = (real *) malloc(M); //结果
    for (int n = 0; n < N2; ++n)
    {
        h_A[n] = n; //对A初始化
    }
    ...
    CHECK(cudaMemcpy(d_A, h_A, M, cudaMemcpyHostToDevice)); //对把A放到gpu上
...
__global__ void transpose1(const real *A, real *B, const int N)
{
    ...
    int bx = blockIdx.x * TILE_DIM; //x方向偏移量
    int by = blockIdx.y * TILE_DIM; //y方向偏移量

    int nx1 = bx + threadIdx.x; //真实位置
    int ny1 = by + threadIdx.y;//真是y
    if (nx1 < N && ny1 < N)
    {
        S[threadIdx.y][threadIdx.x] = A[ny1 * N + nx1]; //合并读取，转置的样子放回共享内存
    }
    __syncthreads(); //一个线程块都放完才往下走 访问共享内存之前，要确保都放进去
